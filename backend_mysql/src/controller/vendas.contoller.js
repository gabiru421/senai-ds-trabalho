...
export const addVendas = (req, res) => {
  ...
  const q =
    "INSERT INTO vendas(`total`, `desconto`, `data_venda` , `produtoID`) VALUES(?)";
  ...
  db.query(q, [values], (err) => {
    ...
    return res.status(200).json("Vneda incluida com sucesso.");
  });
};
...
export const updateVendas = (req, res) => {
  ...
  const q =
    "UPDATE users SET `total` = ?, `desconto` = ?, `data_venda` = ? , `produtoID` = ? WHERE `idvendas` = ?";
  ...
  db.query(q, [...values, req.params.idproduto], (err) => {
    ...
    return res.status(200).json("Vendas atualizada com sucesso.");
  });
};
...
export const deleteVendas = (req, res) => {
  ...
  db.query(q, [req.params.id], (err) => {
    ...
    return res.status(200).json("Vendas deletada com sucesso.");
  });
};